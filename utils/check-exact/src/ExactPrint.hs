...
exactDataFamInstDecl an top_lvl
  ...
  where
    pp_hdr mctxt = do
      markApiAnn an AnnInstance -- TODO: maybe in toplevel
...
exactDataDefn an exactHdr
  ...
  if new_or_data == DataType
    then markApiAnn an AnnData
    else markApiAnn an AnnNewtype
...
instance ExactPrint (HsType GhcPs) where
  ...
  exact x@(HsFunTy an _ _ _)        = withPpr x
...
instance (Outputable a) => ExactPrint (HsScaled GhcPs a) where
  getApiAnnotation = const NoEntryVal
  exact = withPpr

...
signature :: { Located HsModule }
       : maybedocheader 'signature' modid maybemodwarning maybeexports 'where' body
             {% fileSrcSpan >>= \ loc ->
                acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) (AnnsModule [mj AnnSignature $2, mj AnnWhere $6] (fst $7)) cs)
                                            (Just $3) $5 (fst $ snd $7)
                              (snd $ snd $7) $4 $1) )}
...
module :: { Located HsModule }
       : maybedocheader 'module' modid maybemodwarning maybeexports 'where' body
             {% fileSrcSpan >>= \ loc ->
                acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) (AnnsModule [mj AnnModule $2, mj AnnWhere $6] (fst $7)) cs)
                                           (Just $3) $5 (fst $ snd $7)
                              (snd $ snd $7) $4 $1) ) }
        | body2
                {% fileSrcSpan >>= \ loc ->
                   acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) (AnnsModule [] (fst $1)) cs) Nothing Nothing
...
body    :: { (AnnList
        ...
        :  '{'            top '}'      { (AnnList (Just $ moc $1) (Just $ mcc $3) [] (fst $2)
                                         , snd $2) }
        |      vocurly    top close    { (AnnList Nothing Nothing [] (fst $2), snd $2) }

body2   :: { (AnnList
             ,([LImportDecl GhcPs], [LHsDecl GhcPs])) }
        :  '{' top '}'                          { (AnnList (Just $ moc $1) (Just $ mcc $3) [] (fst $2)
                                                  , snd $2) }
        |  missing_module_keyword top close     { (AnnList Nothing Nothing [] [],snd $2) }
...
header  :: { Located HsModule }
        : maybedocheader 'module' modid maybemodwarning maybeexports 'where' header_body
                {% fileSrcSpan >>= \ loc ->
                   acs (\cs -> L loc (HsModule (ApiAnn (realSrcSpan loc) (AnnsModule [mj AnnModule $2,mj AnnWhere $6] (AnnList Nothing Nothing [] [])) cs)
                                              (Just $3) $5 $7 [] $4 $1
                          )) }
...
maybeexports :: { (Maybe (LocatedL [LIE GhcPs])) }
        :  '(' exportlist ')'       {% fmap Just $ amsrl (sLL $1 $> (fromOL $2)) (AnnList (Just $ mop $1) (Just $ mcp $3) [] []) }
        |  {- empty -}              { Nothing }
...
impspec :: { Located (Bool, LocatedL [LIE GhcPs]) }
        ...
        |  'hiding' '(' exportlist ')'      {% do { es <- amsrl (sLL $1 $> $ fromOL $3)
                                                               (AnnList (Just $ mop $2) (Just $ mcp $4) [mj AnnHiding $1] [])
        ...
        : maybe_docnext forall constr_context '=>' constr_stuff
                {% acsa (\cs -> let (con,details,doc_prev) = unLoc $5 in
                  addConDoc (reLocA $ L (comb4 $2 (reLoc $3) $4 $5)
                                         (mkConDeclH98 (ApiAnn (realSrcSpan (comb4 $2 (reLoc $3) $4 $5)) (mu AnnDarrow $4:(fst $ unLoc $2)) cs)
                                                       con
                                                       (snd $ unLoc $2)
                                                       (Just $3)
                                                       details))
        ...
        | maybe_docnext forall constr_stuff
                {% acsa (\cs -> let (con,details,doc_prev) = unLoc $3 in
                  addConDoc (reLocA $ L (comb2 $2 $3)
                                        (mkConDeclH98 (ApiAnn (realSrcSpan (comb2 $2 $3)) (fst $ unLoc $2) cs)
                                                      con
                                                      (snd $ unLoc $2)
                                                      Nothing   -- No context
                                                      details))
...
tup_exprs :: { forall b. DisambECP b => PV ([AddApiAnn],SumOrTuple b) }
           ...
           | commas tup_tail
                 { $2 >>= \ $2 ->
                   do { cos <- mapM (\ll -> amsr (L ll Nothing) [AddCommaAnn (rs ll)]) (fst $1)
                      ; return ([],Tuple (cos ++ $2)) } }
...
commas_tup_tail : commas tup_tail
        { $2 >>= \ $2 ->
          do { cos <- mapM (\l -> amsr (L l Nothing) [AddCommaAnn (rs l)]) (tail $ fst $1)
...
list :: { forall b. DisambECP b => SrcSpan -> (AddApiAnn, AddApiAnn) -> PV (LocatedA b) }
        ...
        | texp '|' flattenedpquals
             { \loc (ao,ac) ->
                checkMonadComp >>= \ ctxt ->
                runECP_PV $1 >>= \ $1 ->
                acsA (\cs -> L loc $ mkHsCompAnns ctxt (unLoc $3) $1 (ApiAnn (realSrcSpan loc) (AnnList (Just ao) (Just ac) [mj AnnVbar $2] []) cs))
                    >>= ecpFromExp' }
...
amsr :: MonadP m => Located a -> [TrailingAnn] -> m (LocatedA a)
amsr a@(L l _) bs = do
  cs <- addAnnsAt l []
